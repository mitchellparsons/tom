...
async function load() {
  if(agentFile.actors) {
    await agentFile.actors.forEach((actorConfig) => {
      let channels = [];
      Object.keys(actorConfig.channels).forEach( async (channelName) => {
      ...
      Object.keys(actorConfig.agency).forEach( async (agencyName) => {
        let agency = await AgencyLoader(agencyName);
